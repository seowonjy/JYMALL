		...
		<div class="content_area">
			...
			<div class="content_top">
				<div class="ct_left_area">
					<div class="image_wrap" data-itemid="${goodsInfo.imageList[0].itemId}" data-path="${goodsInfo.imageList[0].uploadPath}" data-uuid="${goodsInfo.imageList[0].uuid}" data-filename="${goodsInfo.imageList[0].fileName}">
						<img>
					</div>				
				</div>
				<div class="ct_right_area">
					...
					<div class="price">
						<div class="sale_price">정가 : <fmt:formatNumber value="${goodsInfo.itemPrice}" pattern="#,### 원" /></div>
						<div class="discount_price">
							판매가 : <span class="discount_price_number"><fmt:formatNumber value="${goodsInfo.itemPrice - (goodsInfo.itemPrice*goodsInfo.itemDiscount)}" pattern="#,### 원" /></span> 
							[<fmt:formatNumber value="${goodsInfo.itemDiscount*100}" pattern="###" />% 
							<fmt:formatNumber value="${goodsInfo.itemPrice*goodsInfo.itemDiscount}" pattern="#,### 원" /> 할인]
						</div>
						<div>
...
$(document).ready(function(){
	...
	/* cryear */
	const year = "${goodsInfo.cryear}";
	
	let tempYear = year.substr(0,10);
	
	let yearArray = tempYear.split("-")
	let cryear = yearArray[0] + "년 " + yearArray[1] + "월 " + yearArray[2] + "일";
	...
	const itemId = '${goodsInfo.itemId}';	
	$.getJSON("/reply/list", {itemId : itemId}, function(obj){		
...
// 장바구니 추가 버튼
		...
		$.ajax({
			...
			success : function(result){
				if(result === '1'){
					...
					let popUrl = "/replyEnroll/" + memberId + "?itemId=" + itemId;
					console.log(popUrl);
	...
	/* 리뷰 수정 버튼 */
	 $(document).on('click', '.update_reply_btn', function(e){
			...
			let popUrl = "/replyUpdate?replyId=" + replyId + "&itemId=" + '${goodsInfo.itemId}' + "&memberId=" + '${member.memberId}';	
			let popOption = "width = 490px, height=490px, top=300px, left=300px, scrollbars=yes"	
